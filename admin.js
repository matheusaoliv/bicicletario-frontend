...
/* Garantir que modais Bootstrap fiquem acima de tudo */
...
async function carregarMonitoramento(token) {
  ...
  try {
    ...
    console.log('📊 Dados recebidos do backend:', {
      ...
      primeiroFuncionario: funcionarios?.[0],
      todosNomes: funcionarios?.map(f => f.nome)
    });
    ...
    window.graficoRanking = new Chart(ctxRank, {
      type: 'bar',
      data: { labels: rankingBici.map(f => f.nome), datasets: [{ label: 'Movimentações (7 dias)', data: rankingBici.map(f => f.total || 0), backgroundColor: 'rgba(75,192,192,0.7)' }] },
    ...
    (funcionarios || []).forEach((f, idx) => {
      ...
      // Prioriza o campo 'local' ou 'cargo' do Firestore
      if (f.local) {
        local = f.local;
      } else if (f.cargo) {
        const cargoLower = f.cargo.toLowerCase();
        if (cargoLower.includes('bicicletário') || cargoLower.includes('bicicletario')) {
          local = 'Bicicletário';
        } else if (cargoLower.includes('secretaria')) {
          local = 'Secretaria';
        } else {
          local = f.cargo;
        }
      ...
      const rankPos = ranking.findIndex(r => r.nome === f.nome);
      const rankStr = rankPos >= 0 ? `#${rankPos + 1}` : '-';
      tabela.append(`<tr class="${destaque}"><td>${avatar}</td><td>${f.nome}</td><td>${local}</td><td>${statusHtml}</td><td>${lastPingStr}</td><td>${tempoParadoStr}${alertaStr}</td><td>${totalMovStr}</td><td>${ultimaMovStr}</td><td>${rankStr}</td><td>${btnEditar} ${btnExcluir} ${btnDeslogar}</td></tr>`);
    });
      ...
      switch (tabName) {
        ...
        case 'proprietarios':
          if (typeof carregarProprietarios === 'function') {
            // Carregar lista inicial (sem termo de busca)
            console.log('📋 Aba Proprietários aberta - carregando lista...');
            setTimeout(() => {
              const input = document.getElementById('searchProprietarios');
              if (input && !input.value.trim()) {
                // Só carrega todos se o campo estiver vazio
        ...
        case 'auditoria':
          ...
          if (typeof carregarBloqueiosPendentes === 'function') carregarBloqueiosPendentes();
          if (typeof carregarProprietariosBloqueados === 'function') carregarProprietariosBloqueados();
          if (typeof carregarLogsAuditoria === 'function') carregarLogsAuditoria();
...
(function initProprietariosTabela(){
  ...
  window.carregarProprietarios = function(token){
    ...
    console.log('🔍 Buscando proprietários com termo:', termo ? `"${termo}"` : '(todos)');
    alert(`Buscando: ${termo ? termo : 'TODOS'}`); // DEBUG - remover depois
    apiFetchJson('/admin/proprietarios', { query: termo ? { termo } : {} })
      .then(async proprietarios=>{
        console.log('📊 Proprietários recebidos:', {
          total: proprietarios?.length || 0,
          primeiro: proprietarios?.[0],
          campos: proprietarios?.[0] ? Object.keys(proprietarios[0]) : []
        });
        ...
        proprietarios.forEach(p => {
          ...
          // Debug: log do primeiro proprietário
          if (proprietarios.indexOf(p) === 0) {
            console.log('🔍 Debug primeiro proprietário:', {
              nome: p.nome,
              fotoUrl: p.fotoUrl,
              totalBicicletas: bikes.length,
              bicicletas: bikes
            });
          }
          ...
          const nomeHtml = nome
            ? `<a href="#" class="link-historico-prop" data-prop-id="${p.id}" data-prop-nome="${(nome||'').replace(/"/g,'&quot;')}" data-prop-cpf="${String(cpf||'').replace(/"/g,'&quot;')}">${nome}</a>`
            : '-';
    ...
    if(!window._bindBuscaPropsTabela){
      ...
      console.log('🔧 Registrando event listeners dos botões...', { 
        input: !!input, 
        btnBuscar: !!btnBuscar, 
        btnTodos: !!btnTodos 
      });
      ...
      if(btnTodos) {
        btnTodos.addEventListener('click', ()=>{ 
          ...
          alert('Botão LISTAR TODOS clicado!'); 
          if(input) input.value=''; 
          carregarProprietarios(); 
        });
      } else {
        console.error('❌ Botão LISTAR TODOS não encontrado!');
      }
      ...
      window.debugProprietarios = function() {
        console.log('🔍 DEBUG - Estado dos elementos:');
        console.log('Input:', document.getElementById('searchProprietarios'));
        console.log('Botão Buscar:', document.getElementById('btnBuscarProprietarios'));
        console.log('Botão Todos:', document.getElementById('btnListarTodos'));
        console.log('Div Lista:', document.getElementById('proprietariosList'));
        console.log('_bindBuscaPropsTabela:', window._bindBuscaPropsTabela);
      };
...
// Variáveis globais para bloqueios (agora carregadas do backend)
let proprietariosBloqueados = [];
let bloqueiosPendentes = [];
let logsAuditoria = [];

// Função para carregar bloqueios pendentes do backend
async function carregarBloqueiosPendentes() {
  try {
    bloqueiosPendentes = await apiFetchJson('/admin/bloqueios/pendentes');
    renderizarBloqueiosPendentes();
  } catch (error) {
    console.error('Erro ao carregar bloqueios pendentes:', error);
    bloqueiosPendentes = [];
    renderizarBloqueiosPendentes();
  }
}

// Função para carregar proprietários bloqueados do backend
async function carregarProprietariosBloqueados() {
  try {
    proprietariosBloqueados = await apiFetchJson('/admin/bloqueios/ativos');
    renderizarProprietariosBloqueados();
  } catch (error) {
    console.error('Erro ao carregar proprietários bloqueados:', error);
    proprietariosBloqueados = [];
    renderizarProprietariosBloqueados();
  }
}

// Função para carregar logs de auditoria do backend
async function carregarLogsAuditoria() {
  try {
    logsAuditoria = await apiFetchJson('/admin/auditoria', { query: { dias: 30 } });
...
function renderizarBloqueiosPendentes() {
    ...
    bloqueiosPendentes.forEach((bloqueio, idx) => {
        const item = document.createElement('div');
        item.className = 'list-group-item d-flex justify-content-between align-items-center';
        const dataSolicitacao = bloqueio.data_solicitacao ? new Date(bloqueio.data_solicitacao).toLocaleDateString('pt-BR') : 'Data desconhecida';
        item.innerHTML = `
            <div>
                <h6 class="mb-1">${bloqueio.proprietario_nome || 'Nome não disponível'}</h6>
                <small class="text-muted">Solicitado por ${bloqueio.solicitante || 'funcionário'} em ${dataSolicitacao}</small>
                ${bloqueio.motivo ? `<br><small class="text-muted">Motivo: ${bloqueio.motivo}</small>` : ''}
            </div>
            <div>
                <button onclick="confirmarBloqueio('${bloqueio.id}')" class="btn btn-danger btn-sm me-2">
                    <i class="bx bx-block"></i> Confirmar Bloqueio
                </button>
                <button onclick="rejeitarBloqueio('${bloqueio.id}')" class="btn btn-outline-secondary btn-sm">
...
function renderizarProprietariosBloqueados() {
    ...
    proprietariosBloqueados.forEach((bloqueado) => {
        const item = document.createElement('div');
        item.className = 'list-group-item d-flex justify-content-between align-items-center';
        const dataBloqueio = bloqueado.data_bloqueio ? new Date(bloqueado.data_bloqueio).toLocaleDateString('pt-BR') : 'Data desconhecida';
        item.innerHTML = `
            <div>
                <h6 class="mb-1">${bloqueado.nome || 'Nome não disponível'}</h6>
                <small class="text-muted">CPF: ${bloqueado.cpf || 'N/A'}</small><br>
                <small class="text-muted">Bloqueado em ${dataBloqueio} por ${bloqueado.bloqueado_por || 'Admin'}</small>
                ${bloqueado.motivo_bloqueio ? `<br><small class="text-muted">Motivo: ${bloqueado.motivo_bloqueio}</small>` : ''}
            </div>
            <div>
                <button onclick="desbloquearProprietario('${bloqueado.id}')" class="btn btn-success btn-sm">
...
async function confirmarBloqueio(bloqueioId) {
    const bloqueio = bloqueiosPendentes.find(b => b.id === bloqueioId);
    if (!bloqueio) return;

    const confirmacao = confirm(`Confirmar bloqueio do proprietário "${bloqueio.proprietario_nome}"?`);
    if (!confirmacao) return;

    try {
        // Chama API para confirmar bloqueio (você precisa criar essa rota POST)
        await apiFetch('/admin/bloqueios/confirmar', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ bloqueio_id: bloqueioId })
        });

        alert(`Bloqueio confirmado para "${bloqueio.proprietario_nome}"`);
        
        // Recarrega as listas
        await carregarBloqueiosPendentes();
        await carregarProprietariosBloqueados();
        await carregarLogsAuditoria();
    } catch (error) {
        console.error('Erro ao confirmar bloqueio:', error);
        alert('Erro ao confirmar bloqueio. Tente novamente.');
    }
}

// Função para rejeitar bloqueio
async function rejeitarBloqueio(bloqueioId) {
    const bloqueio = bloqueiosPendentes.find(b => b.id === bloqueioId);
    if (!bloqueio) return;

    const confirmacao = confirm(`Rejeitar solicitação de bloqueio do proprietário "${bloqueio.proprietario_nome}"?`);
    if (!confirmacao) return;

    try {
        // Chama API para rejeitar bloqueio (você precisa criar essa rota POST)
        await apiFetch('/admin/bloqueios/rejeitar', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ bloqueio_id: bloqueioId })
        });

        alert(`Solicitação de bloqueio rejeitada para "${bloqueio.proprietario_nome}"`);
        ...
        alert('Erro ao rejeitar bloqueio. Tente novamente.');
    }
}

// Função para desbloquear proprietário
async function desbloquearProprietario(proprietarioId) {
    const proprietario = proprietariosBloqueados.find(p => p.id === proprietarioId);
    if (!proprietario) return;

    const confirmacao = confirm(`Desbloquear o proprietário "${proprietario.nome}"?`);
    if (!confirmacao) return;

    try {
        // Chama API para desbloquear (você precisa criar essa rota POST)
        await apiFetch('/admin/bloqueios/desbloquear', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ proprietario_id: proprietarioId })
        });

        alert(`Proprietário "${proprietario.nome}" foi desbloqueado`);
        ...
        alert('Erro ao desbloquear proprietário. Tente novamente.');
    }
}
...
function loadAssistLogo(){
  try {
    ...
    img.onerror = function(){
      // Silenciosamente ignora erro de CORS ao abrir via file://
      window.assistPdfLogo = null;
    };
