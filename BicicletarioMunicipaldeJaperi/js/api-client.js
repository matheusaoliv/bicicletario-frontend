...
(function(global){
  ...
  function getStoredToken(){
    try { return sessionStorage.getItem('token'); } catch(_){ return null; }
  }
  ...
  async function request(method, path, body, opts={}){
    ...
    let url = path.startsWith('http') ? path : API_BASE_URL.replace(/\/$/,'') + path;
    // Append compat=1 for admin endpoints
  ...
  // Helper: ilike client-side (case-insensitive contains)
  function ilikeMatch(value, query){
    const v = (value==null?'' : String(value)).toLowerCase();
    const q = (query==null?'' : String(query)).toLowerCase();
    return v.includes(q);
  }

  global.api = { get, post, put, del, upload, request, setStoredToken, getStoredToken, clearStoredToken, endpoints, ilikeMatch };
