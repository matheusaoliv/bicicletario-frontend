...
// Usa a variável global ou define se não existir
window.API_BASE_URL = window.API_BASE_URL || 'https://southamerica-east1-bicicletario-japeri-v3.cloudfunctions.net/api';
...
function inicializarDetalheRegistro() {
  ...
  // Inicializar selects dependentes no card (se módulo estiver disponível)
  try {
    if (window.CatalogoBikesUI) {
      ...
      CatalogoBikesUI.setupDependentSelects({
        ...
        marcaSel: '#detMarca',
        modeloSel: '#detModelo',
        marcaSearch: '#detMarcaSearch',
        modeloSearch: '#detModeloSearch',
        marcaClear: '#detMarcaClear',
        modeloClear: '#detModeloClear',
        marcaOutroWrap: '#detMarcaOutroWrap',
        modeloOutroWrap: '#detModeloOutroWrap',
        marcaOutroInput: '#detMarcaOutro',
        modeloOutroInput: '#detModeloOutro',
        marcaSuggestions: '#detMarcaSuggestions',
        modeloSuggestions: '#detModeloSuggestions',
        includeOutroOption: true,
        includeLinhaFilter: true,
        linhaFilterWrap: '#detLinhaFilterWrap',
        linhaSearch: '#detLinhaSearch',
        linhaClear: '#detLinhaClear',
        linhaSuggestions: '#detLinhaSuggestions'
      });
    }
  } catch(_){ }
}
...
function exibirCardComDados(card, item) {
  ...
  // Prefill selects dependentes com dados da bike atual
  try {
    const tSel = document.getElementById('detTipoBike');
    const mSel = document.getElementById('detMarca');
    const mdSel = document.getElementById('detModelo');
    const marcaOutroInp = document.getElementById('detMarcaOutro');
    const modeloOutroInp = document.getElementById('detModeloOutro');
    const tipoVal = item?.bicicleta?.tipo_bike || '';
    const marcaVal = item?.bicicleta?.marca || '';
    const modeloVal = item?.bicicleta?.modelo || '';
    if (tSel) {
      tSel.value = tipoVal || '';
      tSel.dispatchEvent(new Event('change'));
    }
    // tentar setar marca e modelo após popular
    function trySetMarcaModelo(attempt){
      const att = attempt||0;
      if (att>12) return;
      if (mSel && !mSel.hasAttribute('disabled')){
        // Marca
        if (marcaVal){
          const has = Array.from(mSel.options||[]).some(o=>o.value===marcaVal);
          if (has){ mSel.value = marcaVal; mSel.dispatchEvent(new Event('change')); }
          else { mSel.value='Outro'; mSel.dispatchEvent(new Event('change')); if(marcaOutroInp) marcaOutroInp.value = marcaVal; }
        }
        // Modelo (após marca)
        setTimeout(()=>{
          if (mdSel && !mdSel.hasAttribute('disabled')){
            if (modeloVal){
              const hasMd = Array.from(mdSel.options||[]).some(o=>o.value===modeloVal);
              if (hasMd){ mdSel.value = modeloVal; mdSel.dispatchEvent(new Event('change')); }
              else { mdSel.value='Outro'; mdSel.dispatchEvent(new Event('change')); if(modeloOutroInp) modeloOutroInp.value = modeloVal; }
            }
          } else { setTimeout(()=> trySetMarcaModelo(att+1), 80); }
        }, 100);
      } else {
        setTimeout(()=> trySetMarcaModelo(att+1), 80);
      }
    }
    setTimeout(()=> trySetMarcaModelo(0), 120);
  } catch(_){ }
  ...
  if (btnAcao) {
    btnAcao.onclick = async () => {
      ...
      if (acao==='Check-out' && bikeObj.status==='FORA') { window.mostrarPopupConfirmacao('Esta bicicleta está FORA. Não há registro para check-out.'); return; }
      ...
      try {
        ...
        const obsOcorrencia = document.getElementById('detalheObservacoesOcorrencia');
        if (acao==='Check-in') {
          // Validar número do lacre
          const lacreEl = document.getElementById('numeroLacre');
          const lacreRaw = lacreEl ? String(lacreEl.value||'').trim() : '';
          const lacre = lacreRaw.replace(/\D+/g,'');
          if(!lacre){ window.mostrarPopupConfirmacao('Informe o número do lacre antes de confirmar o Check-in.'); throw new Error('Número do lacre obrigatório'); }
...
function setCamposEditaveis(editavel) {
  ...
  // Habilitar/desabilitar selects dependentes
  const tSel = document.getElementById('detTipoBike');
  const mSel = document.getElementById('detMarca');
  const mdSel = document.getElementById('detModelo');
  const sEls = [tSel, mSel, mdSel, document.getElementById('detMarcaSearch'), document.getElementById('detModeloSearch'), document.getElementById('detMarcaClear'), document.getElementById('detModeloClear'), document.getElementById('detLinhaSearch'), document.getElementById('detLinhaClear')];
  sEls.forEach(el=>{ if(!el) return; if(editavel) el.removeAttribute('disabled'); else el.setAttribute('disabled',''); });
  const obsNormais = document.getElementById('detalheObservacoesNormais');
  const obsOcorrencia = document.getElementById('detalheObservacoesOcorrencia');
...
async function onEditarBtnClick() {
  ...
  if (!editavel) {
    ...
    // Capturar valores finais dos selects dependentes (considerando 'Outro')
    const tipoSel = document.getElementById('detTipoBike');
    const marcaSel = document.getElementById('detMarca');
    const modeloSel = document.getElementById('detModelo');
    const marcaOutro = document.getElementById('detMarcaOutro')?.value.trim() || '';
    const modeloOutro = document.getElementById('detModeloOutro')?.value.trim() || '';
    const tipoVal = (tipoSel && tipoSel.value) || '';
    const marcaSelVal = (marcaSel && marcaSel.value) || '';
    const modeloSelVal = (modeloSel && modeloSel.value) || '';
    const marca = (tipoVal==='Outro' || marcaSelVal==='Outro') ? marcaOutro : (marcaSelVal || '');
    const modelo = (tipoVal==='Outro' || marcaSelVal==='Outro' || modeloSelVal==='Outro') ? modeloOutro : (modeloSelVal || '');
    ...
    try {
      ...
      if (bicicletaId) {
        ...
        if (tipoVal) payload.tipo_bike = tipoVal;
        await api.put(`/bicicletas/${bicicletaId}`, payload).catch(err=>{ window.mostrarPopupConfirmacao(err.message || 'Erro ao atualizar bicicleta.'); });
      }
...
function aplicarBicicletaSelecionada(){
  ...
  // Sincroniza selects dependentes no card
  try {
    const tSel = document.getElementById('detTipoBike');
    const mSel = document.getElementById('detMarca');
    const mdSel = document.getElementById('detModelo');
    const marcaOutroInp = document.getElementById('detMarcaOutro');
    const modeloOutroInp = document.getElementById('detModeloOutro');
    if (tSel) { tSel.value = bike.tipo_bike || ''; tSel.dispatchEvent(new Event('change')); }
    function trySet(attempt){
      const att = attempt||0; if(att>12) return;
      if (mSel && !mSel.hasAttribute('disabled')){
        if (bike.marca){
          const has = Array.from(mSel.options||[]).some(o=>o.value===bike.marca);
          if (has){ mSel.value = bike.marca; mSel.dispatchEvent(new Event('change')); }
          else { mSel.value='Outro'; mSel.dispatchEvent(new Event('change')); if(marcaOutroInp) marcaOutroInp.value = bike.marca; }
        }
        setTimeout(()=>{
          if (mdSel && !mdSel.hasAttribute('disabled')){
            if (bike.modelo){
              const hasMd = Array.from(mdSel.options||[]).some(o=>o.value===bike.modelo);
              if (hasMd){ mdSel.value = bike.modelo; mdSel.dispatchEvent(new Event('change')); }
              else { mdSel.value='Outro'; mdSel.dispatchEvent(new Event('change')); if(modeloOutroInp) modeloOutroInp.value = bike.modelo; }
            }
          } else { setTimeout(()=> trySet(att+1), 80); }
        }, 100);
      } else { setTimeout(()=> trySet(att+1), 80); }
    }
    setTimeout(()=> trySet(0), 120);
  } catch(_){ }
  ajustarBotaoAcaoPrincipal();
