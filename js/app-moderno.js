  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderizarResultados(resultados){
      ...
      resultados.forEach(item => {
        const status = item.status || (item.registro_entrada_atual ? 'DENTRO' : (item.bicicleta?.id ? 'FORA' : 'SEM_BICICLETA'));
      ...
      if (btnAddBike) {
        btnAddBike.onclick = async ()=>{
          ...
          try {
            ...
            fd.append('proprietario_id', String(pid));
            fd.append('numero_bike', numero);
            fd.append('tipo_bike', tipo || '');
            fd.append('marca', marca);
            fd.append('modelo', modelo);
            fd.append('observacoes', obs || '');
            if(fotoBike) fd.append('fotoBicicleta', fotoBike);
            if(fotoDono) fd.append('fotoDonoComBicicleta', fotoDono);
            const token = getToken();
    ...
    async function executarAcaoPrincipal(){
      ...
      const status = itemAtivoNoModal.status || (itemAtivoNoModal.registro_entrada_atual ? 'DENTRO' : (itemAtivoNoModal.bicicleta?.id ? 'FORA' : 'SEM_BICICLETA'));
      const estaDentro = status === 'DENTRO';
      ...
      try {
        if(estaDentro){
          ...
          if (!lacreVerificado) {
            showOpPopup('Verificação Pendente', 'Você precisa verificar o lacre antes de continuar.', { mode: 'error' });
            return;
          }
          ...
          const lacreRaw = ui.modal.numeroLacre ? String(ui.modal.numeroLacre.value || '').trim() : '';
          const lacre = lacreRaw.replace(/\D+/g, ''); // mantém apenas dígitos
          if(!lacre){
            const nome = itemAtivoNoModal.proprietario?.nome_completo || '-';
            const cpf = itemAtivoNoModal.proprietario?.cpf || '';
            const details = `
              <div class="op-detail-row"><strong>Proprietário:</strong> ${escapeHtml(nome)} ${cpf?`(CPF: ${escapeHtml(cpf)})`:''}</div>`;
            showOpPopup('Falha no Check-in', 'Informe o número do lacre antes de confirmar.', { mode: 'error', detailsHtml: details, autoCloseMs: 3200 });
            return;
          }
