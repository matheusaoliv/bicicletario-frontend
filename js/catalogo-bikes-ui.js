(function(){
  ...
  const API_BASE = ((GLOBAL.API_BASE_URL && GLOBAL.API_BASE_URL.trim()) || 'https://southamerica-east1-bicicletario-japeri-v3.cloudfunctions.net/api').replace(/\/$/, '');
  ...
  CatalogoBikesUI.loadCatalog = async function({url}={}){
    if(_cache) return _cache; if(_loading) return _loading;
    const primary = url || (API_BASE + '/catalogo-bikes');
    const fallbacks = [ primary, 'data/catalogo-bikes.json' ];
    _loading=(async()=>{
      for(const u of fallbacks){
        try { const r=await fetch(u,{cache:'no-store'}); if(r.ok){ const d=await r.json(); if(d&&typeof d==='object'){ _cache=d; return d; } } }
        catch(_){ }
      }
